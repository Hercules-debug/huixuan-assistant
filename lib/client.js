...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 取已登记的图片引用。
     *
     * 来源是 host 侧 `presentationMeta` 写入的 `block.meta.images`，
     * 而不是 `block.content` —— 因为当前模型是纯文本的，图片刻意不进
     * 模型可见的结果内容，只走 UI 元数据。
     */
    function resultImageRefs(block) {
      ...
      return list.filter((r) => r && typeof r === 'object' && typeof r.attachmentId === 'string');
    }
