...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 依赖的客户端服务。
     *
     * ⚠️ `remote` 是父服务，必须与 `remote.credentials` **分别列出** ——
     * 只写 `'remote.credentials'` 时访问 `ctx.remote` 会抛
     * `cannot get property "remote" without inject`。
     * 内置卡片（dsh-client-ui-settings-plugins）也是两个都列。
     */
    ...
    const S = {
      ...
      diag: {
        ...
        background: 'var(--dsw-alias-bg-warning, #fffbeb)',
        border: '1px solid var(--dsw-alias-border-warning, #fcd34d)',
      ...
      /** 商品图卡片：比设置卡更宽松，图片需要横向空间 */
      gallery: { width: '100%', maxWidth: '520px' },
      /** 工具结果原文：等宽、可换行、限高 */
      pre: {
        margin: 0, fontSize: '12px', lineHeight: 1.6,
        whiteSpace: 'pre-wrap', wordBreak: 'break-word',
        fontFamily: 'var(--dsw-font-mono, ui-monospace, SFMono-Regular, Menlo, monospace)',
        opacity: 0.85, maxHeight: '320px', overflow: 'auto',
      },
    };

    /**
     * 由设置值推断当前会走哪一档凭证。
     * 注意：仅凭 clientId/pid 无法确认密钥是否就绪，所以措辞保留余地。
     */
    function levelLabel(value) {
      const hasBoth = Boolean(value?.clientId && value?.pid);
      return hasBoth ? '独立（密钥齐全时生效）' : '共享（零配置）';
    }
    ...
    function HuixuanCard(props) {
      ...
      const [secretState, setSecretState] = React.useState({
        configured: false, writable: true, diag: null,
      });
       ...
       * 读密钥状态。
       *
       * 这里刻意保留原始响应与错误文本（diag），因为「未配置」有两种截然不同的成因：
       *   a) 服务确实说没配（configured:false）
       *   b) 这次 RPC 本身失败了（权限、方法名、返回结构不符）
       * 若把两者都显示成「未配置」，用户会以为自己没配过而重复填写。
       */
        ...
        try {
          ...
          if (!r || r.ok !== true) {
            setSecretState({
              ...
              diag: `describe 调用未成功：${JSON.stringify(r)?.slice(0, 200) ?? String(r)}`,
            });
            return;
          }
          const view = r.value?.[SECRET_REF];
          if (view === undefined) {
            setSecretState({
              configured: false, writable: true,
              diag: `describe 返回里没有 ${SECRET_REF}：${JSON.stringify(r.value)?.slice(0, 200)}`,
            });
            return;
          }
          setSecretState({
          ...
          setSecretState({
            configured: false, writable: true,
            diag: `describe 抛错：${e?.message ?? String(e)}`,
          });
        }
      }, [ctx]);
      ...
      children.push(h(Field, {
        ...
        placeholder: secretState.configured
          ? '已配置（留空则不改动）'
          : (secretState.diag ? '状态未知（见下方诊断）' : '尚未配置'),
        hint: '只保存在本机 DSH 凭据库中，不会上传、不会进入对话或日志。',
      }));

      children.push(h('div', { key: 'status', style: S.btnRow }, [
        h('span', { key: 'b1', style: S.badge },
          secretState.configured ? '密钥：已配置'
            : (secretState.diag ? '密钥：状态未知' : '密钥：未配置')),
        h('span', { key: 'b2', style: S.badge },
          writable ? '设置：可写' : '设置：只读'),
        h('span', { key: 'b3', style: S.badge },
          `凭证级别：${levelLabel(remoteValue)}`),
      ]));

      // 密钥状态诊断：只在 RPC 异常/结构不符时显示，避免误导用户重复填写
      if (secretState.diag) {
        children.push(h('div', { key: 'diag', style: S.diag }, [
          h('div', { key: 't', style: { fontWeight: 600, marginBottom: '2px' } },
            '密钥状态读取诊断（请把这段发给我）'),
          h('div', { key: 'v' }, secretState.diag),
        ]));
      }
    ...
    /**
     * 取已登记的图片引用（content 优先，meta 兜底）。
     */
    function resultImageRefs(block) {
      const isRef = (r) => r && typeof r === 'object' && typeof r.attachmentId === 'string';

      // 主来源：block.content。这不只是渲染需要，更是**授权需要** ——
      // 宿主下发附件前会做引用校验（session.attachment → referencedImage），
      // 那个校验只扫 content 系列位置，**从不扫 block.meta**。
      // 所以图块必须由 host 侧写进 render 的 content。
      const content = Array.isArray(block?.content) ? block.content : [];
      const fromContent = content
        .filter((b) => b && b.type === 'image' && b.attachment && typeof b.attachment === 'object')
        .map((b) => b.attachment)
        .filter(isRef);
      if (fromContent.length > 0) return fromContent;

      // 兜底：早期版本只写 presentationMeta
      const meta = block?.meta;
      if (meta && typeof meta === 'object' && !Array.isArray(meta) && Array.isArray(meta.images)) {
        return meta.images.filter(isRef);
      }
      return [];
    }
     ...
     *
     * props 由工具视图框架注入：
     * `{ callId, toolName, block, openFile, cwd, home, loadImage, inspect, renderSlot, t }`
     */
    function ShopImagesRow(props) {
      const { block, loadImage, renderSlot } = props;
      const text = resultText(block);
      const refs = resultImageRefs(block);

      const children = [];

      if (refs.length > 0) {
        // 复用内置图库 slot（由 @deepseek-ai/dsh-client-ui-attachment 提供），
        // 避免自己实现缩放/预览/加载态。
        const gallery = (renderSlot && loadImage)
          ? renderSlot('tool.call.images', {
              images: refs.map((ref) => ({ attachment: ref })),
              loadImage,
              align: 'start',
            })
          : h('div', { style: S.hint }, `（${refs.length} 张商品图，但图片组件不可用）`);
        children.push(h('div', { key: 'gallery', style: S.gallery }, gallery));
      }

      if (text) {
        children.push(h('pre', { key: 'text', style: S.pre }, text));
      }

      if (children.length === 0) {
        children.push(h('div', { key: 'empty', style: S.hint }, '（无内容）'));
      }
    ...
    /** 注册商品图卡片到指定工具名 */
    function registerToolView(ctx, toolName) {
      try {
        ctx.slots.inject('tool.call.toolview', () => ctx.slots.register({
          name: 'tool.call.toolview',
          key: toolName,
          // 声明子 slot 才能在图卡里派发图库（与内置 read_image 视图同款）
          children: { 'tool.call.images': { kind: 'single', scope: 'session' } },
        }, ShopImagesRow));
      } catch (e) {
        console.error(`[huixuan-assistant] 注册 ${toolName} 工具视图失败：`, e);
      }
    }
    ...
    function apply(ctx) {
      ...
      // 商品图卡片：接管所有会带图的工具。
      // shop_search 也带图（前 N 件），因为电商场景下用户默认就想看商品长什么样。
      registerToolView(ctx, 'shop_search');
      registerToolView(ctx, 'shop_detail');
      registerToolView(ctx, 'shop_compare');
    }
