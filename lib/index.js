...
export function apply(ctx, config) {
  ...
  // 注册设置命名空间：注册后 ~/.dsh/settings.yaml 里的 huixuan-assistant 段
  // 才会成为用户层。设置服务不可用时降级为环境变量。
  const settingsSvc = service('settings');
  let scope;
  try {
    scope = settingsSvc?.register?.(SETTINGS_NS, SettingsSchema);
  } catch (err) {
    ctx.logger?.warn?.(`huixuan-assistant: 设置命名空间注册失败，改用环境变量：${err?.message}`);
    scope = undefined;
  }
   ...
   * 读取用户配置。
   *
   * - clientId / pid 走 settings 命名空间（非密钥）
  ...
  async function readCreds() {
    let settings = {};
    try {
      if (scope?.get) settings = scope.get() ?? {};
      else if (settingsSvc?.get) settings = settingsSvc.get(SETTINGS_NS) ?? {};
    } catch { /* 设置不可用，忽略 */ }
