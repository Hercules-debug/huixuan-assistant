...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
       ...
       * UI 侧的图片引用。
       *
       * ⚠️ 刻意不放 render()：render 的内容会发给模型，而当前模型是纯文本的
       * （不声明 image 输入），塞图片块进去要么被投影成文本、要么报错。
       * presentationMeta 只进 block.meta，由客户端的 tool view 读取渲染。
       */
      presentationMeta: (_args, value) => (
        value.ok && value.goods?.image_ref
          ? { images: [value.goods.image_ref] }
          : {}
      ),
    },
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      /** 同 shop_detail：图片只走 presentationMeta，不给模型。 */
      presentationMeta: (_args, value) => {
        const images = (value.goods ?? [])
          .filter((g) => g.image_ref)
          .map((g) => g.image_ref);
