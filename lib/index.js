...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: '查询单个拼多多商品的详细信息（价格、原价、销量、店铺、服务标签等），'
      + '并附带商品主图 —— 主图会由工具卡片自动显示给用户，你无需在回答里贴图片链接。'
      + '**用户明确要看某件商品或其图片时直接调用本工具**（一次调用只消耗 1 次配额），'
      + '不要先反问一轮。'
      + '⚠️ 每次调用会额外下载一张商品图（约数十至数百 KB），不要对整页搜索结果逐个调用。',
    parameters: {
      ...
      systemPrompt.section({
        ...
        text: ({ scope }) => {
          ...
          return [
            '【慧选助手 · 商品工具使用约定】',
            '',
            '1. 展示商品图：`shop_search` 只返回文字列表（价格/销量/品牌/服务标签），不带图。',
            '   用户想看某件商品的图时，**直接调用 `shop_detail`** —— 商品主图会由工具卡片自动显示。',
            '   ⚠️ 不要在回答里写 `![名称](图片URL)` 或粘贴图片链接来「展示」商品图：',
            '   DSH 的对话界面不渲染远程图片链接，那样只会显示成一串无法点击的死链。',
            '   要展示图，唯一办法就是调用 `shop_detail`（多件用 `shop_compare`）。',
            '',
            '2. 配额：`shop_search` 一次调用返回整页，很省；但 `shop_detail` / `shop_compare`',
            '   对每个商品各消耗一次调用（个人账号约 10 次/分钟），所以不要对整页结果逐个调用。',
            '   但**不要因此把「确认」变成额外一轮**：',
            '   · 用户已明确指定商品时（「看第一个」「对比这 3 个」「有图吗」）→ **直接调用**；',
            '   · 用户只是泛泛地说「帮我看看充电宝」而没指定 → 先列候选让用户挑。',
            '',
            '3. 数据边界：拼多多联盟接口**不返回商品规格参数**（容量、材质、尺寸、屏幕等）。',
            '   不要编造这些信息；用户需要参数时，请引导其查看商品页。',
            '   参数只能从标题里提取（如「20000毫安」「22.5W」），提取时请说明这是从标题读出的。',
            '',
            '4. 数据来源：拼多多联盟官方 API，**仅覆盖可推广商品**，不是全平台商品库。',
